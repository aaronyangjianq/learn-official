        ...
        <div class="container">
          <div class="row align-center">
            <div class="align-center block height border pt-pb">
              <div class="pf pr">
                <p>SDK名称：&nbsp;{{creatOrder.sdk && creatOrder.sdk.title}}{{creatOrder.sdk && creatOrder.sdk.version}}</p>
                <p>最近更新日期：&nbsp;{{creatOrder.sdk && creatOrder.sdk.update_time}}</p>
              </div>
              <div class="pb">
                <p class="p1">订单金额：</p>
              </div>
              <div class="pb">
                <p class="p2">¥{{creatOrder.price}}</p>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  async created() {
    const id = this.$route.query.versionid;
    const rawData = await post('/home/order/create', {
      version_id: id
    });

    this.creatOrder = rawData.data;
  },
  components: {},
  methods: {
    async toPay() {
      const orderId = this.creatOrder.id;
      ...
      if (paytype === 1) {
        ...
        window.location.href = `${this.$store.state.baseUrl}/home/payment/pay?id=${orderId}&pay_type=${paytype}`;
      }
    },
    choosePay(id) {
      this.num = id;
